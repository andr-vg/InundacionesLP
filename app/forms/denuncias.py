...
class CreateDenunciaForm(FlaskForm):
    ...
    title = StringField(
        "Titulo", [validators.DataRequired(message="Debe ingresar un titulo")]
    )
    category = SelectField(
        "Categoria",
        [validators.DataRequired(message="Debe seleccionarse una categoria")],
        coerce=int,
    )
    ...
    lastname = StringField(
        ...
        [
            ...
            validators.regexp(
                "^[a-zA-Z]+$", message="Debe ingresar un apellido valido"
            ),
        ],
    )
    tel = StringField(
        ...
        [
            ...
            validators.DataRequired(message="Debe ingresar un teléfono valido"),
        ],
    )
    email = StringField(
        "Email",
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args)
        self.category.choices = [(0, "Sin asignar")] + [
            (category.id, category.name) for category in Categoria.get_all()
        ]
        self.user.choices = [(0, "Sin asignar")] + [
            (user.id, user.email) for user in User.get_with_state(User.get_all(), True)
        ]
        if kwargs:
    ...
    def validate_user(form, field):
        ...
        if not (field.data in choices):
            raise validators.ValidationError(
                "Usuario invalido, elija un usuario cargado en el sistema"
            )
    ...
    def validate_category(form, field):
        ...
        if field.data == 0:
            raise validators.ValidationError(
                "Categoría inválida, elija una categoria valida"
            )
        if not (field.data in choices):
            raise validators.ValidationError(
                "Categoría inválida, elija una categoria valida"
    ...
    def validate_state(form, field):
        ...
        if not (field.data in choices):
            form.category.errors = (
                validators.ValidationError("Estado inválido, elija un estado valida"),
            )
